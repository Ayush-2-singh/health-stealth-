  ...
  export default function Home() {
    return (
      <>
        ...
        <section className="relative py-24 px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
              {stats.map((stat, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: i * 0.1 }} className="relative bg-white border border-border rounded-2xl p-8 text-center hover:border-primary/20 hover:shadow-card transition-all shadow-soft">
                  <div className="text-4xl md:text-5xl font-bold gradient-text-blue mb-2">{stat.value}<span className="text-secondary">{stat.suffix}</span></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: i * 0.1 }} className="bg-white border border-border rounded-2xl p-8 hover:border-primary/20 hover:shadow-card transition-all shadow-soft group">
                  <div className="w-12 h-12 bg-primary/8 border border-primary/15 rounded-xl flex items-center justify-center mb-5 group-hover:bg-primary/12 transition-colors">
        ...
        <section className="relative py-24 px-6 lg:px-8 bg-surface">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="text-center mt-12">
              <Link href="/technology" className="inline-flex items-center gap-2 text-secondary hover:text-secondary-dark font-medium transition-colors">View All Technologies <span>→</span></Link>
            </div>
          </div>
        </section>
        ...
        <section className="relative py-24 px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="text-center mt-12">
              <Link href="/products" className="inline-flex items-center gap-2 text-secondary hover:text-secondary-dark font-medium transition-colors">Explore Full Catalog <span>→</span></Link>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              {applications.map((app, i) => (
                <motion.div key={app.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.08 }} whileHover={{ y: -4 }} className="group relative bg-white border border-border rounded-2xl p-6 hover:border-primary/20 hover:shadow-card transition-all shadow-soft">
                  <div className="w-12 h-12 bg-gradient-to-br from-primary/8 to-secondary/5 rounded-xl flex items-center justify-center mb-4">
                    <HiGlobe className="w-6 h-6 text-primary" />
                  </div>
                  <h3 className="text-lg font-bold text-heading mb-2 group-hover:text-primary transition-colors">{app.title}</h3>
                  <p className="text-xs text-gray-600 leading-relaxed line-clamp-3">{app.description}</p>
                </motion.div>
              ))}
            </div>
          </div>
              ...
              {researchAreas.slice(0, 3).map((area, i) => (
                <GlassCard key={area.id} delay={i * 0.1}>
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 bg-primary/8 border border-primary/15 rounded-xl flex items-center justify-center flex-shrink-0">
                      <HiAcademicCap className="w-6 h-6 text-primary" />
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-heading mb-2">{area.title}</h3>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {[
                ...
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="relative bg-white border border-border rounded-xl p-5 text-center hover:border-primary/20 hover:shadow-soft transition-all shadow-soft">
                  <step.icon className="w-8 h-8 text-primary mx-auto mb-3" />
                  <div className="text-xs text-gray-500 uppercase tracking-wider">{step.label}</div>
                  {i < 5 && <div className="hidden lg:block absolute top-1/2 -right-2 w-4 h-px bg-border" />}
                </motion.div>
              ))}
            </div>
          </div>
