...
export default function Button({ href, onClick, variant = "primary", size = "md", children, className, type = "button" }: ButtonProps) {
  ...
  const variants = {
    primary: "bg-primary hover:bg-primary-light text-white shadow-lg hover:shadow-glow",
    secondary: "bg-secondary hover:bg-secondary-light text-white shadow-lg hover:shadow-glow-green",
    ghost: "bg-white/5 hover:bg-white/10 text-white border border-border hover:border-white/20",
  ...
  const content = (
    <>
      ...
      {variant === "primary" && <span className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700" />}
    </>
  );
