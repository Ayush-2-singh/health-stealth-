    ...
    >
      <div className="relative h-48 bg-surface overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <span className="px-2.5 py-1 text-[11px] font-semibold text-gray-700 bg-white border border-border rounded-full">
            {product.category}
          </span>
        </div>
      </div>
      <div className="p-6">
        ...
        <p className="text-sm text-gray-700 leading-relaxed mb-5 line-clamp-2 font-normal">{product.description}</p>
        <div className="flex flex-wrap gap-1.5 mb-5">
          {product.features.slice(0, 3).map((feature) => (
            <span key={feature} className="px-2 py-0.5 text-[11px] text-gray-700 bg-surface border border-border rounded-md font-medium">
              {feature}
            </span>
          ))}
        </div>
        <Link
          ...
          className="inline-flex items-center gap-1.5 text-sm font-semibold text-secondary hover:text-secondary-dark transition-colors duration-150 group/link"
        >
          Learn More
